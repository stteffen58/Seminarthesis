...
class LSH(object):
    ...
    def query(self, query_point, num_results=None, distance_func=None):
        ...
        for ix in candidates:
			...
			dist = d_func(query_point, self._as_np_array(point))
    ...
    def cosine_dist(x, y):
		x1 = x[0].toarray()
		y1 = y[0].toarray()
		return 1 - np.dot(x1[0], y1[0]) / ((np.dot(x1[0], x1[0]) * np.dot(y1[0], y1[0])) ** 0.5)
		#return scipydist.cosine(x[0].toarray()[0],y[0].toarray()[0])
        #return 1 - x.dot(y.T) / ((x.dot(x.T) * y.dot(y.T))**0.5)
